...
def qualification():
    ...
    if request.method == 'POST':
        ...
        if len(kmlData) == 0:
            ...
            for placemark in kmlData.getElementsByTagName('Placemark'):
                ...
                k.write("qualification: KML shape = %s\n" % polygon)

                # Attempt to convert from KML to ***REMOVED*** geom format.
                try:
                    # Store training polygons into qual_user_maps.
        ...
        if assignmentStatus is None:
            if score < hitAcceptThreshold:
                ...
                qualForm.resultsAccepted.data = 2   # Indicate rejected results.
            else:
                assignmentStatus = MappingCommon.HITApproved
                qualForm.resultsAccepted.data = 1   # Indicate approved results.
    ...
    # If GET request, tell showkml.js to not issue any alerts.
        ...
        qualForm.resultsAccepted.data = 0   # Indicate GET (i.e., no results alert in showkml)

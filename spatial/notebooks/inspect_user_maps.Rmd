...
### Arguments
```{r}
read_dbase <- FALSE  # grab updated data? TRUE of FALSE
...
save(grids, kml_assn_hits, file = "../data/test/inspect_user_maps.rda")
```
...
if(!read_dbase) {
  ...
  load("../data/test/inspect_user_maps.rda")
}
```
...
# scene
scene <- tbl(con, "scenes_data") %>% filter(cell_id == grid_sel$id) %>%
  collect()
...
f <- "../data/test/qsites_digitization_tests.csv"
data.table::fwrite(grid_selo, file = f, append = TRUE)
save(scene, user_polys, gpoly, grid_sel, 
     file = "../data/test/inspect_user_maps_data.rda")
```

```{r, eval=TRUE, echo=FALSE, results="markup"}
if(new_sample) {
  print(paste0("Selected new assignment: ", grid_sel$assignment_id))
}
if(!new_sample) {
  load("../data/test/inspect_user_maps_data.rda")
  print(paste0("Re-visiting previous assignment ", grid_sel$assignment_id))
}
```
...
if(!is.null(user_polys)) {
  m <- m %>% addPolygons(data = user_polys, fill = FALSE,
                         color = "yellow", group = "Maps", weight = 2)
} else {
  m <- m
}

...
```{r, eval = FALSE}
dfunc <- function(x) {
  length(which(abs(diff(as.vector(x))) > 1))
}
...
# s <- sample(1:nrow(lowest_sortdt), size = 20, replace = FALSE)
# dry_wins <- do.call(rbind, lapply(s, function(x) {  # x <- 386434
dry_wins <- do.call(rbind, lapply(1:nrow(lowest_sortdt), function(x) {  
  dat <- unlist(lowest_sortdt[x, ])
  v <- dat[3:length(dat)]
  # v <- c(1, 3, 11, 12)
  # # v <- c(1, 10:12)
  # # v <- c(1:6)
  ...
  if(vd != 0) { 
    if(any(any(v <= 3) & any(v >= 10))) {  # dry straddling the new year
       v2 <- v
...
dry_winsdt <- data.table(dry_wins)
dry_winsdtf <- cbind(lowest_sortdt[, .(x, y)], dry_winsdt)
setnames(dry_winsdtf, c("V1", "V2"), c("d1", "d2"))

registerDoMC(7)
dry_wins2 <- foreach(i = 1:7, .combine = "rbind") %dopar% {  # i <- 1
  DT <- lowest_sortdt[block == i]
  DT[, block := NULL]
  dry_win_block <- do.call(rbind, lapply(1:nrow(DT), function(x) {
    dat <- unlist(DT[x, ])
    v <- dat[3:length(dat)]
    vd <- dfunc(v)
    if(vd == 0) {  # dry months all contiguous and in Southern Hemisphere 
...
dry_winsdt2 <- data.table(dry_wins2)
dry_winsdtf2 <- cbind(lowest_sortdt[, .(x, y)], dry_winsdt2)
setnames(dry_winsdtf2, c("V1", "V2"), c("d1", "d2"))

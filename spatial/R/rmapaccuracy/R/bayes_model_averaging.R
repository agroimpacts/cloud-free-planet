...
bayes_model_averaging <- function(bayes.polys, rasterextent, threshold) {
  ...
  for (t in 1:nrow(bayes.polys)) {
    ...
    if (t == 1) {
      weight_acc <- weight 
      posterior_acc <- overlay(posterior_field_rst, weight, 
  ...
  label_map <- reclassify(heat_map, c(-Inf, threshold, 0, threshold, Inf, 1)) 
  
  # risks maps is to assign non-field (1-label_map) as heat_map values, and 
  # asssign field as 1-heat_map
  risk_map <- overlay(heat_map, label_map, 
                      fun=function(r1, r2) {r1 *(1 - r2) + (1 - r1) * r2})
  
  return(list("labelmap" = label_map, "heatmap" = heat_map, 
              "riskmap" = risk_map))
}
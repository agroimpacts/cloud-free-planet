...
KMLAccuracy <- function(mtype, kmlid, assignmentid, tryid, 
  ...
  diam<-0.005/2 # new grid diameter
  ...
  # qaqc.sql <- paste0("select gid, geom_clean, geom", 
  #                   " from qaqcfields_SY where name=", "'", kmlid, "'") #testing line
  ...
  if(qaqc.hasfields == "Y") {
    ...
    if (st_crs(qaqc.polys)$epsg==4326){ # if qaqc fields are based on GCS, then need to transform them to PCS
  ...
  if(user.hasfields == "Y") {  # Read in user fields if there are any
    if(any(st_is_empty(user.polys))) {  # invoke cleaning algorithm 
        user.polysclean <- cleanTempPolyFromWKT(unfixedsfc=st_as_sfc(structure(list(user.polys$geom), class = "WKB"), EWKB = TRUE), 
                                               crs = gcs)
        user.nfields <- nrow(user.polysclean)  #  Record n distinct fields 
        user.poly <- st_union(st_transform(user.polysclean,crs=prjstr)) # need to transform user polygons into pcs for calculation
  ...
  if((qaqc.hasfields == "N") & (user.hasfields == "N")) {
    ...
    # drv_sand <- dbDriver("PostgreSQL") #testing line
    # con_sand <- dbConnect(drv_sand, dbname = "AfricaSandbox", user = "***REMOVED***", password = '***REMOVED***') #testing line
    # xy_tabs<-data.table(con_sand %>% tbl("master_grid") %>% filter(name==kmlid)%>% collect()) # testing line

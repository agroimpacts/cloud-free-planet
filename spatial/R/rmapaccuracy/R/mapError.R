...
mapError <- function(maps, truth, region) {
  if(is.null(nrow(truth))) {
    ...
    tn <- st_difference(null, maps)  # True negative: do polygon differencing across IDs
  } 
  if(is.null(nrow(maps))) {
  ...
  if(!is.null(nrow(truth)) & !is.null(nrow(maps))) {

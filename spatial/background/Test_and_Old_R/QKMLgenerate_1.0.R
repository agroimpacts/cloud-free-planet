#! /usr/bin/R -f 
##############################################################################################################
# Title      : QKMLgenerate_1.0.R
# Purpose    : KML file generation script for QAQC grid cells
# Author     : Dennis McRitchie, Lyndon Estes
# Draws from : KMLgenerate*.R
# Used by    : 
# Notes      : Created 20/10/2012
#              This writes out the existing 609 QKMLs sites in one go
##############################################################################################################
...
log.hdr <- rbind("Error messages from QKMLGenerate_X.X.R", 
                 "Time gridID afmap_errcode afmap_errmessage SouthAfrica_errcode SouthAfrica_errmessage",
                 "#####################################################################################")
setwd(log.file.path)
write.table(log.hdr, file = "RQKMLGenerate.log", sep = "", col.names = F, row.names = F, quote = FALSE)
options(digit.secs = 4)  # Display milliseconds for time stamps 

# Step 1. Poll the database (newqaqc_sites) to get grid IDs
grid.IDs <- dbGetQuery(con2, "select id from newqaqc_sites")
grid.IDs <- grid.IDs[, 1]

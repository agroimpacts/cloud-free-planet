...
prjsrid       <- 97490  # EPSG identifier for equal area projection
...
if(test.root == "N") {
  ...
  if(qaqc.hasfields == "Y") {
    ...
    #if(as.character(gIsValid(qaqc.polys))
  ...
  if((qaqc.hasfields == "N") & (user.hasfields == "N")) {
    ...
    #     if(as.character(gIsValid(qaqc.polys)) == "FALSE") {
  ...
  if((qaqc.hasfields == "N") & (user.hasfields == "Y")) {
    ...
    user.poly.in <- gIntersection(spgeom1 = grid.poly, spgeom2 = user.poly, byid = TRUE)  ### Turker maps in

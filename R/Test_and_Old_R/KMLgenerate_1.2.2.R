...
log.file.path <- dbGetQuery(con, "select value from configuration where key = 'LogFilePath'")
log.file.path <- log.file.path$value
...
log.hdr <- rbind("Error messages from KMLGenerate_X.X.R", 
                 ...
                 "#####################################################################################")
setwd(log.file.path)
write.table(log.hdr, file = "RKMLGenerate.log", sep = "", col.names = F, row.names = F, quote = FALSE)
# write("test", file = "RKMLGenerate.log", append = T)
...
repeat {
  ...
  min.avail.kml <- min.avail.kml$count
  ...
  if(avail.kml.count < min.avail.kml) {  # Set pretty high in case HITs are drawn down at rapid rate
    ...
    grid.IDs <- dbGetQuery(con2, "select id, fwts from sa1kgrid where avail = 'T'")
    ...
    sql <- paste("SELECT ST_AsEWKT(geom) from sa1kgrid where id in", " (", paste(id.rand, collapse = ","), 
                 ")", sep = "")
	   id.geom <- dbGetQuery(con2, sql)  # Get the coordinates for the random grid
    ...
    ret2 <- dbSendQuery(con2, paste("UPDATE sa1kgrid SET avail='F' where ID in", 
                                    "(", paste(geom.tab[, 1], collapse = ","), ")", sep = ""))
    
    # Database error handling
		  exception <- dbGetException(con)  # afmap update exceptions
	   exception2 <- dbGetException(con2)  # South Africa update exceptions
    ...
    # Crude solution built into your original code:
		  if ((exception$errorNum != 0) | (exception2$errorNum != 0)) {

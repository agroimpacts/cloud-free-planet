...
# cbind(a[-length(a)], a[-1])
# length(a) == nrow(flds.rand)

newIDs[a, ] <- flds.rand  # Splice in fields every 3rd row
# length(which(is.na(newIDs$name))) == nrow(nflds.rand)
newIDs[is.na(newIDs$name), ] <- nflds.rand  # Fill in 2 non-fields between every field
#nrow(newIDs[ newIDs$fields == "Y", ]) + nrow(newIDs[ newIDs$fields == "N", ])
